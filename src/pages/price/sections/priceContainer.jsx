...
const PriceContainer = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`${BASE_URL}/products`)
      .then((data) => {
        const sortPrice = sort ? data.data.sort((a, b) => (a.price - b.price)) : data.data.sort((a, b) => (a.price - b.price)).reverse();
        const result = sortPrice.filter(resultPrice => resultPrice.category === cate);

        if(cate === 'ALL'){
            if(keyword){
                const allKeyword = sortPrice.filter(search => createFuzzyMatcher(keyword).test(search.name));
                setPrice(allKeyword);
        ...
        if(cate !== 'ALL'){
            if(keyword){
                const keyWordResult = result.filter(search => createFuzzyMatcher(keyword).test(search.name));
  ...
  return (
    <>
      <PriceHeader price={price} orgPrice={orgPrice} setPrice={setPrice} setCate={setCate}  cate={cate} setKeyword={setKeyword}/>
